...
> Hsieh, H.-W. (2026). *TCM-Meridian (杏林經緯): A multi-agent, safety-oriented AI clinical assistant for Traditional Chinese Medicine* (v1.0.0). Zenodo. https://doi.org/10.5281/zenodo.20725779
...
- **看診工作流**：檔案式患者與就診 Session 管理、三欄式工作台、病歷瀏覽/編輯與 Snapshot 版本控制（undo / redo / diff）、人工修改自動標記，就診日期附 50 字 NOTE 摘要索引。
- **多智能體協作**：ReAct 主 Agent 以嚴格 JSON 契約協調病歷登載、幻覺審查、低信心標註、病歷檢查員、問診助理等 Subagent，並可諮詢教授 RAG（query expansion → 前綴分類 → 檢索 → RRF → rerank → 回答）、讀取患者影像與文字檔；全程記錄於「智能體互動行為」時間線。
...
主介面左欄的「選取日期」下拉會以 NOTE 摘要輔助辨識，例如 `2026-06-11（頭痛、喉嚨痛）`。「選擇模板」下拉不顯示摘要，維持純日期列表。

「摘要並退出」只退出目前就診日期，不退出患者，且只作用於**已確認載入**的日期（未載入時提示先確認日期）。「刪除 session」則作用於**下拉框目前選取**的日期（可不必載入該日期）；刪除非載入中的日期時會保留載入中的病歷畫面。若主 Agent 正在執行、問診子流程進行中，或病歷仍在編輯模式，系統會鎖住「摘要並退出」按鈕以避免狀態或草稿遺失。相同忙碌保護也套用於切換患者、切換 session、新增/刪除 session 與退出患者。摘要模型未設定或呼叫失敗，會 fallback 使用原文前 50 字。

歷史摘要的生成與審查/重寫共用同一個子模型（`main_agent.history_summary`）；若摘要產生失敗（未設定模型或呼叫失敗），新增 session 的 UI 會以 ⚠️ 明示失敗，而非誤報「已產生」。
...
- **建議由人類專家執行切割**，使每個父段是「一段具有連續性、難以再切割的醫學文本段落」。父段字數建議1000~3000字，若文本太長且難以切割則建議維持原長度(例，某段落具有嚴密的前後邏輯故難以切割，但字數達8000餘字，建議保留這8000餘字為完整父段)。
