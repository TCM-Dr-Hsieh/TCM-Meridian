...
**替換結果三態**：每筆 phrase 回傳 `replaced`（在未標註區找到並新增標註）／`already_tagged`（只在既有 `**…**` 內找到，判定已處理、不重複標）／`not_found`（未標註原文中找不到）。只有 `not_found` 進「先前各輪曾識別但無法成功標註」的失敗清單要求下一輪重新擷取；`already_tagged` 進「已處理」摘要、不再被當成定位失敗。`total_annotated` 只累計 `replaced`。
...
- 低信心標註防巢狀（跳過已標註區），且替換結果三態化（`replaced`/`already_tagged`/`not_found`），僅 `not_found` 進失敗重試清單。
