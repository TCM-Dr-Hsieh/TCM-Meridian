...
Every `call_professor.action_input` must include the JSON boolean `show_forum_history`. With `false`, existing forum history is not passed to the professor; its prompt shows an explicit system-hidden notice instead, supporting forum-blinded independent analysis and separately collected second opinions. With `true`, the complete forum is visible for critique, comparison, disagreement resolution, or continuation. Missing or invalid values safely default to `false`. This switch controls only forum-history visibility: it does not clear the professor's graffiti wall or ReAct work history, and prior opinions already present in NOTE, A&T, or other visible context may still exert indirect influence, so `false` is not a fully isolated blind review. Every successful consultation is still appended to the forum. The behavior timeline, step record, RAG log, and human-readable forum text log record the visibility used for the call; legacy posts without this field are marked as unrecorded.
